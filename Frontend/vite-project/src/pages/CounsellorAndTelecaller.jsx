...
const PROGRESS_FIELDS = [
  { id: "registrationFeeSubmitted", label: "Registration Fee Submitted", group: "regdocs" },
  { id: "documentsSubmitted",       label: "Documents Submitted",        group: "regdocs" },
  { id: "documentFileReady",        label: "Document File Ready",        group: "admission" },
  { id: "collegeApplicationDone",   label: "College Application Done",   group: "admission" },
  { id: "admissionLetterIssued",    label: "Admission Letter Issued",    group: "admission" },
...
const StudentFullDetails = ({ student }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-5 bg-slate-50 p-5 rounded-2xl border">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            <Badge variant={lead.variant} className="text-xs">{lead.label}</Badge>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
        {[
          { icon: <Phone className="h-4 w-4 text-slate-400" />, label: "Phone",            value: student.phone },
          { icon: <UsersIcon className="h-4 w-4 text-slate-400" />, label: "Parent Name",  value: student.parentName },
          { icon: <MapPin className="h-4 w-4 text-slate-400" />, label: "City",            value: student.city },
          { icon: <IndianRupee className="h-4 w-4 text-slate-400" />, label: "Budget",     value: student.budget ? `₹${Number(student.budget).toLocaleString("en-IN")}` : null },
          { icon: <Tag className="h-4 w-4 text-slate-400" />, label: "Lead Type",          value: lead.label },
          { icon: <BookOpen className="h-4 w-4 text-slate-400" />, label: "NEET Status",   value: student.neetStatus },
          { icon: <Calendar className="h-4 w-4 text-slate-400" />, label: "Pref. Country", value: student.preferredCountry },
          { icon: <User className="h-4 w-4 text-slate-400" />, label: "Current Status",    value: student.status },
      ...
      {/* Extra */}
      <div className="pt-4 border-t grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
        <div><span className="text-slate-500">College: </span>{student.collegeName || "—"}</div>
        <div><span className="text-slate-500">Emergency Contact: </span>{student.emergencyContact || "—"}</div>
        <div><span className="text-slate-500">Service Manager: </span>{student.serviceManager || "—"}</div>
        <div><span className="text-slate-500">Last Follow-up: </span>{formatDate(student.lastFollowUp)}</div>
      </div>
    </div>
  );
};
...
const Counsellorandtellecaller = () => {
  const [activeTab, setActiveTab]     = useState("interested");
  const [students, setStudents]       = useState({ interested: [], followUp: [], closed: [] });
  const [loading, setLoading]         = useState(true);
  ...
  const fetchLeads = async () => {
    try {
      ...
      setStudents({
        interested: all.filter((l) => ["New", "Interested"].includes(l.status)),
        followUp:   all.filter((l) => ["Call Back"].includes(l.status)),
        closed:     all.filter((l) => ["Converted", "Admitted", "Dropped", "Not Interested"].includes(l.status)),
      });
    } catch (err) {
      toast.error("Failed to load students. Please login again.");
      console.error(err);
  ...
  const openProgressModal = (student) => {
    // Pre-fill from existing data if backend stores progress fields
    setProgressData({
      registrationFeeSubmitted: student.registrationFeeSubmitted || false,
      ...
      remarks:                  student.progressRemarks          || "",
    });
    setProgressModal(student);
  };
  ...
  const saveProgress = async () => {
    ...
    try {
      ...
      const { remarks, ...checkboxes } = progressData;
  ...
  const saveFollowUp = async () => {
    ...
    try {
      ...
      const newStatus = moveToStatus || remarkModal.status;
          ...
          data.map((student) => {
            ...
            return (
              <TableRow key={student._id} className="hover:bg-muted/50">
                ...
                <TableCell className="text-sm text-muted-foreground">
                  {formatDate(student.lastFollowUp)}
                </TableCell>
                <TableCell>
                  <div className="flex flex-wrap gap-2 justify-center">
                    <Button variant="outline" size="sm" onClick={() => openRemarkModal(student)}>
                      <Edit2 className="h-4 w-4 mr-1" />
                      Remark
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => openProgressModal(student)}>
                      <Eye className="h-4 w-4 mr-1" />
                      Progress
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => setDetailModal(student)}>
                      <User className="h-4 w-4 mr-1" />
                      Details
                    </Button>
                  </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8 space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-2">Student Pipeline &amp; Admission Progress Management</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: "Total Students",   count: Object.values(students).flat().length, bg: "bg-blue-100",   icon: <Users className="h-8 w-8 text-blue-600" /> },
          { label: "Interested",        count: students.interested.length,             bg: "bg-amber-100",  icon: <Users className="h-8 w-8 text-amber-600" /> },
          { label: "Follow-up",         count: students.followUp.length,               bg: "bg-orange-100", icon: <Clock className="h-8 w-8 text-orange-600" /> },
          { label: "Closed / Admitted", count: students.closed.length,                 bg: "bg-emerald-100",icon: <UserCheck className="h-8 w-8 text-emerald-600" /> },
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full max-w-md grid-cols-3 bg-white border">
          <TabsTrigger value="interested"><Users className="h-4 w-4 mr-1" />Interested</TabsTrigger>
          <TabsTrigger value="followUp"><Clock className="h-4 w-4 mr-1" />Follow-up</TabsTrigger>
          <TabsTrigger value="closed"><UserCheck className="h-4 w-4 mr-1" />Closed</TabsTrigger>
        </TabsList>

        <div className="mt-8">
          {[
            { key: "interested", title: "Interested Students" },
            { key: "followUp",   title: "Follow-up Queue" },
            { key: "closed",     title: "Closed / Admitted Students" },
          ].map(({ key, title }) => (
            <TabsContent key={key} value={key} className="m-0">
              <Card>
                <CardHeader><CardTitle>{title}</CardTitle></CardHeader>
                <CardContent>{renderTable(key, students[key])}</CardContent>
              </Card>
            </TabsContent>
          ))}
        </div>
          ...
          <div className="space-y-8 py-6">
            ...
            <div>
              ...
              <div className="space-y-3 pl-6">
                {PROGRESS_FIELDS.filter((f) => f.group === "regdocs").map((item) => (
                  <div key={item.id} className="flex items-center space-x-3">
                    <Checkbox
                      id={item.id}
                      checked={progressData[item.id]}
                      onCheckedChange={(v) => setProgressData((p) => ({ ...p, [item.id]: v }))}
                    />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                <h3 className="font-semibold text-lg mb-4">Admission Process</h3>
                <div className="space-y-3 pl-6">
                  {PROGRESS_FIELDS.filter((f) => f.group === "admission").map((item) => (
                    <div key={item.id} className="flex items-center space-x-3">
                      <Checkbox
                        id={item.id}
                        checked={progressData[item.id]}
              ...
              <div>
                <h3 className="font-semibold text-lg mb-4">Visa &amp; Departure</h3>
                <div className="space-y-3 pl-6">
                  {PROGRESS_FIELDS.filter((f) => f.group === "visa").map((item) => (
                    <div key={item.id} className="flex items-center space-x-3">
                      <Checkbox
                        id={item.id}
                        checked={progressData[item.id]}
            ...
            <div>
              ...
              <Textarea
                value={progressData.remarks}
                onChange={(e) => setProgressData((p) => ({ ...p, remarks: e.target.value }))}
                placeholder="Any special observations, issues, or next steps..."
      ...
      <Dialog open={!!remarkModal} onOpenChange={() => setRemarkModal(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Follow-up — {remarkModal?.name}</DialogTitle>
            <DialogDescription>Add your remark and optionally move the student</DialogDescription>
          </DialogHeader>

          <div className="space-y-5 py-4">
            <div>
              <Label>Remark / Follow-up Note <span className="text-red-500">*</span></Label>
              <Textarea
                value={remark}
                onChange={(e) => setRemark(e.target.value)}
                placeholder="What was discussed? Next steps, objections, etc..."
            ...
            <div>
              ...
              <Select value={moveToStatus} onValueChange={setMoveToStatus}>
                ...
                <SelectContent>
                  <SelectItem value="Interested">Interested</SelectItem>
                  <SelectItem value="Call Back">Call Back (Follow-up)</SelectItem>
                  <SelectItem value="Converted">Converted / Admitted</SelectItem>
          ...
          <DialogFooter>
            ...
            >
              {savingRemark
                ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</>
                : <><Save className="mr-2 h-4 w-4" />Save Follow-up</>
              }
            </Button>
          </DialogFooter>
        </DialogContent>
